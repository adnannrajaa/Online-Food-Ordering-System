...
let getOrdersData = () => {
    $.ajax({
        url: '/Admin/GetTodaysOrders',
        type: 'Get',
        contentType: 'application/json; charset=utf-8',
        dataType: "json",
        success: function (response) {
            loadData(response.result.Order)
            updateHeaderRow(response.result)
        },
...
let updateHeaderRow =(headerData)=>{
    $("#delivered-orders").text(headerData.OrderDelivered);
    $("#pending-orders").text(headerData.OrderPending);
    $("#total-items").text(headerData.TotalItems);
    $("#total-users").text(headerData.TotalUsers);
